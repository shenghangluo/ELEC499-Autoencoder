import tensorflow as tf
import numpy as np
import keras.backend as K
from scipy.io import loadmat

class Channel(object):
    def __init__(self, inputs, alpha, name='channel'):
        with tf.name_scope(name):
            # input shape is (?, 1, 104)
            # Upsampling
            self._upsampled = tf.contrib.layers.fully_connected(inputs, num_outputs=536, activation_fn=None,
                                                             weights_initializer=my_init, biases_initializer=None, trainable=False)

            # Slice it to real part and imaginary part and do the convolution separately
            # and concate them to get the output
            self._upreal = tf.slice(self._upsampled, [0, 0, 0], [-1, -1, 268])
            self._upimage = tf.slice(self._upsampled, [0, 0, 268], [-1, -1, 268])

            # Convolution with Root Raised Cosine Function
            shift = np.random.randint(-4, 4)
            filt = loadmat('rrc.mat')['rrt']
            filt = np.array(filt)
            filt = filt.flatten()[62 - 15 + shift:62 + 16 + shift]
            filt = tf.convert_to_tensor(filt, dtype=tf.float32)
            filt = tf.reshape(filt, [31, 1, 1])

            # Convolution with real part
            self._transposed_real = tf.transpose(self._upreal, [0, 2, 1])
            self._convd_real = tf.nn.conv1d(self._transposed_real, filters=filt, padding='VALID')
            self._output_real = tf.transpose(self._convd_real, [0, 2, 1])

            # Convolution with imaginary part
            self._transposed_image = tf.transpose(self._upimage, [0, 2, 1])
            self._convd_image = tf.nn.conv1d(self._transposed_image, filters=filt, padding='VALID')
            self._output_image = tf.transpose(self._convd_image, [0, 2, 1])

            # Concate the real and imaginary part
            #self._output = tf.concat([self._output_real, self._output_image], 2)

            #self._output = tf.reshape(self._output, [-1, 476])


            # Multiply with Complex number (Rotation)
            print("self._output_real shape is ", self._output_real.shape)
            print("self._output_image shape is: ", self._output_image.shape)
            real = tf.reshape(self._output_real, [-1, 238])
            imag = tf.reshape(self._output_image, [-1, 238])

            sequence = np.arange(1, 239, 1)
            sequence = sequence * 2 * np.pi

            fi = np.random.normal(0, 11.75, size=238)
            np.clip(fi, -20, 20)
            fi_off = np.random.uniform(0, 2 * np.pi, size=238)

            theta = np.multiply(sequence, fi)
            theta = np.add(theta, fi_off)

            sin_theta = np.float32(np.sin(theta))
            cos_theta = np.float32(np.cos(theta))
            # print("sin_theta", sin_theta)
            # print("cos_theta", cos_theta)

            sin_theta = tf.convert_to_tensor(sin_theta)
            cos_theta = tf.convert_to_tensor(cos_theta)

            final_real = tf.math.subtract(tf.math.multiply(real, cos_theta), tf.math.multiply(imag, sin_theta))
            print("final_real shape is: ", final_real.shape)
            final_image = tf.math.add(tf.math.multiply(real, sin_theta), tf.math.multiply(imag, cos_theta))
            print("final_image shape is: ", final_image.shape)

            self._output = tf.concat([final_real, final_image], 1)

    def get_ChannelOuput(self):
        return self._output

def my_init(shape, dtype=None, partition_info=None):
    val = np.zeros((104, 536))
    row_index = np.arange(52)
    colum_index = np.arange(30, 238, 4)
    val[row_index, colum_index] = 1.0

    row_index = np.arange(52, 104)
    colum_index = np.arange(298, 506, 4)
    val[row_index, colum_index] = 1.0

    return K.variable(value=val, dtype=dtype)
