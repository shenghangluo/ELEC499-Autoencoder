...
class Model(object):
    def __init__(self, dataset_params, encoder_params, channel_params, decoder_params, decision_params):
        ...
        self._channelout = self._channel.get_ChannelOuput()     # shape (?, 1, 476)


        # AWGN noise layer   -----Channel Model(Part)
        self._w = noi.gaussian_noise_layer(input_layer=self._channelout, std=self.Noise)
        #print("w shape is: ", w.shape)

        #print("w before complex is: ", w.shape)

        # convert to complex number and then slice
        self._w = RToC.get_c(self._w)
        #print("w after complex is: ", w.shape)

        # Slicer
        self.slice_output = tf.slice(self._w, [0, k1], [-1, k2 - k1 + 1])
        ...
        self._u = self._decoder.get_outputs()
        # decision      -----Receiver Model
        self._decision = decis.Decision(inputs=self._u, **decision_params)
    ...
    def get_para(self):
        return self._encoder.get_central()
