...
class DatasetMNIST(object):
    ...
    def load_data(self):
        ...
        for i in range(TRAIN_MESS):
            X_train_all[i, :, :] = X_train[random.randint(0, 255), :, :]

        X_train_final = np.zeros((TRAIN_MESS - 12, 13, 256))
        for i in range(TRAIN_MESS - 12):
            k = i
            for j in range(group_num):
                X_train_final[i, j, :] = X_train_all[k, :, :]
                k = k + 1

        for i in range(VALID_MESS):
            X_valid_all[i, :, :] = X_train[random.randint(0, 255), :, :]

        X_valid_final = np.zeros((VALID_MESS - 12, 13, 256))
        for i in range(VALID_MESS - 12):
            k = i
            for j in range(group_num):
                X_valid_final[i, j, :] = X_valid_all[k, :, :]
                k = k + 1
